...
if($q !== ''){
  $like = '%'.$q.'%';
  $stmt = $conn->prepare("SELECT * FROM books WHERE title LIKE ? OR author LIKE ? OR isbn LIKE ? OR category LIKE ? ORDER BY title LIMIT 50");
  $stmt->bind_param('ssss',$like,$like,$like,$like);
  $stmt->execute(); $rows = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
} else {
  $rows = $conn->query("SELECT * FROM books ORDER BY id DESC LIMIT 12")->fetch_all(MYSQLI_ASSOC);
}
?>
...
<body>
<header class="header">
  <img src="assets/images/ewu_library_logo.jpg" alt="logo"><h1>Search Books</h1>
</header>
<div class="container">
<form method="get"><input name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Search by title, author, ISBN"> <button class="btn">Search</button></form>
<table class="table" style="margin-top:12px">
<tr><th>Title</th><th>Author</th><th>ISBN</th><th>Category</th><th>Available</th><th>Action</th></tr>
...
<tr>
  ...
  <td><?= (int)$r['available_copies'] ?>/<?= (int)$r['total_copies'] ?></td>
  <td>
    <?php if(isset($_SESSION['user'])): ?>
      <form action="actions/borrow.php" method="post" style="display:inline">
        <input type="hidden" name="book_id" value="<?= (int)$r['id'] ?>">
        <button class="btn">Borrow</button>
      </form>
      <form action="actions/reserve.php" method="post" style="display:inline">
        <input type="hidden" name="book_id" value="<?= (int)$r['id'] ?>">
        <button class="btn outline">Reserve</button>
      </form>
    <?php else: ?>
      <a class="btn outline" href="login.php">Login to borrow</a>
    <?php endif; ?>
  </td>
</tr>
<?php endforeach; ?>
